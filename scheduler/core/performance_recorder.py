...
# Local
import numpy as np
from Jingle.scheduler.core.bank import Bank
from Jingle.scheduler.core.base_performance_recorder import BasePerformanceRecorder

logger = logging.getLogger(__name__)


NUM_DATA_FOR_RECENT_REPORTS = 3


class PerformanceRecorder(BasePerformanceRecorder):
    ...
    @classmethod
    def _compute_vals_with_data_batch(cls, data_batch, fields_to_report, pr):
        ...
        for elem in data_batch:
            ...
            for fld in fields_to_report:
                if fld == "allocs":
                    batch_tot_vals[fld] += elem[fld][pr.descr] * curr_time
                else:
                    batch_tot_vals[fld] += elem[fld] * curr_time
    ...
    def _update_vals_with_new_data(self, new_data):
        ...
        for elem in new_data:
            ...
            for fld in self.fields_to_report:
                if fld == "allocs":
                    self._curr_total_vals[fld] += elem[fld][self.descr] * curr_time
                else:
    ...
    def fetch_data_and_update_history(self):
        ...
        if new_data:
            ...
            self._recent_mean_vals = self._compute_vals_with_data_batch(
                self.history[-NUM_DATA_FOR_RECENT_REPORTS:], self.fields_to_report, self)
...
class PerformanceRecorderBank(Bank):
    ...
    def _check_type(cls, obj):
        """ Checks type. """
        assert isinstance(obj, BasePerformanceRecorder)
...
#                         logger.info(err_str)
    ...
    def get_recent_allocs_and_util_metrics(self, num_recent_event_logs, grid_size):
        ret = []
        for key, mspb in self.enumerate():
            logger.info(f'metrics in performance recorder {mspb._recent_mean_vals}')
            curr_val = {
                'alloc': {key: mspb._recent_mean_vals['allocs']},
                'loads': {key:mspb._recent_mean_vals['load']},
                'leaf_rewards': {key:mspb._recent_mean_vals['reward']}, # used to capture p99 latency slo
            }
            ret.append(curr_val)
        return ret
    ...
    #             'alloc': {key: mspb._recent_mean_vals['allocs']},
    #             'loads': {key:mspb._recent_mean_vals['load']},
    #             'leaf_rewards': {key:mspb._recent_mean_vals['load']/mspb._recent_mean_vals['allocs']/30}, # used to capture (throughput) request for each pod
    #         }
    #         ret.append(curr_val)
    #     return ret
