...
from Jingle.scheduler.core.performance_recorder import PerformanceRecorderBank
...
from Jingle.utility.events import AllocExpirationEvent, AppAddEvent, AppUpdateEvent, BaseEvent, \
    EventTypes, UtilityUpdateEvent
from Jingle.utility.hierarchy_env import TreeEnvironment
...
PERF_DEBUG = True if 'Jingle_PERF_DEBUG' in os.environ else False

class JingleScheduler:
    """ main scheduler. """

    def __init__(self,
                 event_queue: asyncio.Queue,
                 framework_manager: KubernetesManager,
                 event_logger: object,
                 env: object = None,
        ...
        """
        Main Jingle scheduler class. Responsible for processing events in the provided event queue
        The Jingle scheduler also reads the environment (kubernetes) and creates
        ...
        :param framework_manager: Framework manager (k8s)
    ...
    def processor_alloc_expiration_event(self, event: AllocExpirationEvent):
        ...
        if PERF_DEBUG:
            ...
            with open('./Jingle_perf.txt', 'a') as f:
    ...
    def _add_app_to_env(self, app_path, app_threshold, app_weight, app_unit_demand):
        ...
        if not (self.data_logger_bank.tag_exists(app_path)):
            logger.info('Adding %s to data logger bank in JingleScheduler.', app_path)
            data_logger = DataLogger(app_path, DATALOGGER_FIELDS,
        ...
        if self.add_learners_if_not_given and \
            ...
            logger.info('Adding %s to load forecaster bank in JingleScheduler.', app_path)
        ...
        if self.add_learners_if_not_given and \
            ...
            logger.info('Adding %s to learner bank in JingleScheduler.', app_path)
    ...
    def processor_utility_event(self, event: UtilityUpdateEvent):
        ...
        # logger.info("--------- tag is ----------- " + event.app_path)
