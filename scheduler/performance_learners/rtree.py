"""
    Utilities in estimating and constructing confidence intervals for a multi-parameter function.
"""
...
MIN_NUM_DATA_IN_NODE_BEFORE_EXPANDING = 2
...
#         y_range = glob_upper_bound - glob_lower_bound
    ...
    def get_recommendation_for_lower_bound(self, perf_goal):
        """ Obtain a recommendation for estimating lower bound. """
        # pylint: disable=arguments-differ
...
#             last_node.num_data_in_node))
    ...
    #             loc.append(v)
    #     X = [temp/load for temp, load in zip(loc, Loads)]
    #     for x, y, sigma in zip(X, Rewards, Loads):
    #         self.add_data_point(x, y, 1/sigma)

    # for throughput expriments
    def update_model_with_new_data(self, Allocs, Rewards, Loads, Sigmas, _):
        """ Updates model with new data. """
        loc = []
        for alloc in Allocs:
