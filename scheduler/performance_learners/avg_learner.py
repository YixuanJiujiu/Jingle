"""
    A learner for the average latency.
...
class AvgLearner(BaseLearner):
    ...
    def _fetch_and_format_latest_data(self):
        ...
        new_data, new_time_stamp = \
            self.data_logger.get_data(fields=['load', 'avg_latency', 'allocs',
                                              'event_start_time', 'event_end_time'],
                                      start_time_stamp=self.current_time_stamp,
                                      end_time_stamp=None)
        Allocs = [elem['allocs'] for elem in new_data]
        Rewards = [-elem['avg_latency'] for elem in new_data]
