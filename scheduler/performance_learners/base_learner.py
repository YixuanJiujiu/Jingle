...
# local
from Jingle.scheduler.performance_learners.learner_bank import DemandRecommender
...
class BaseLearner(DemandRecommender):
    ...
    # def _fetch_and_format_latest_data(self):
    #     """ Fetches and formats the latest data.
    #         Overrides BaseLearner to set the Reward, Allocs to a
    #         dictionary of allocations for each leaf.
    #     """
    #     SLO_throughput = 5000  # hardcoded the slo requirements for coding assignment applications 1s
    #     new_data, new_time_stamp = \
    ...
    #                                   end_time_stamp=None)
    #     Allocs = [elem['allocs'] for elem in new_data]
    #     Rewards = [SLO_throughput - elem['load']/30/elem['allocs']['root--assign-distribute'] for elem in new_data]
    #     Loads = [elem['load'] for elem in new_data]
    #     Sigmas = None
    #     Event_times = [elem['event_end_time'] - elem['event_start_time'] for elem in new_data]
    #     return Allocs, Rewards, Loads, Sigmas, Event_times, new_time_stamp
    ...
    def model_update_routine(self):
        ...
        if num_data_received == 0:
            return
        # logger.info("----------------fetch and format----------Allocs: " + str(Allocs) + "-----Rewards: " + str(
        #     Rewards) + "-------Loads: " + str(Loads))
