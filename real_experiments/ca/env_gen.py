...
def generate_env_test2(real_or_dummy):
    """ Generate query environment. """
    # job_dict = {ms: {'threshold': 1, 'util_scaling': 'linear', 'workload_type': real_or_dummy}
    #             for ms in MICROSERVICES}
    # return get_flat_environment_from_dict_of_jobs(job_dict, real_or_dummy)
    c1_info = {'name': 'assignment-query', 'threshold': 1, 'util_scaling': 'linear', 'workload_type': 'assignment-query'}
    # Create environment --------------------------------------
    root = InternalNode('root')
    child1 = _get_leaf_node_from_info_dict(c1_info, real_or_dummy)
    root.add_children([child1], [1])
...
def generate_env_simple(real_or_dummy):
    ...
    c2_info = {'name': 'assignment-query', 'threshold': 1, 'util_scaling': 'linear', 'workload_type': 'assignment-query'}
    # Create environment --------------------------------------
    root = InternalNode('root')
    child1 = _get_leaf_node_from_info_dict(c1_info, real_or_dummy)
