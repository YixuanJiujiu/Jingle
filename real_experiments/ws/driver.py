...
from Jingle.scheduler.performance_learners.sgd import SGDLearner
from Jingle.scheduler.performance_learners.p99_learner import P99Learner
...
from Jingle.scheduler.allocation_policies.as_multi import K8sMultiScaler, PidMultiScaler
from Jingle.scheduler.allocation_policies.sensing_policy import MJingleAutoScaler
...
def main():
    ...
    for leaf_path, leaf in env.leaf_nodes.items():
        ...
        if args.policy in ['multijingle', 'k8sas', 'pid']:
        ...
        if args.policy in ['multijingle']:
            # learning model for each SLO -----------------------------------------------------------------
    ...
    if args.policy == 'multijingle':
        policy = MJingleAutoScaler(
        ...
        policy = K8sMultiScaler(
            env=env, resource_quantity=num_resources,
            performance_recorder_bank=performance_recorder_bank)
    elif args.policy == 'pid':
        policy = PidMultiScaler(env=env, resource_quantity=num_resources,
            performance_recorder_bank=performance_recorder_bank)
    else:
