...
from Jingle.utility.kube_manager import load_k8s_config
...
class AllocationBasedDataSource():
    def __init__(self,
    ...
    def get_data(self) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"KeyError occurred in get_allocation. Returning none and not publishing data. Error: {str(e)}")
            return None
        data = {
...
class K8sProportionalDataSource(AllocationBasedDataSource):
    ...
    def get_allocation(self):
        ...
        if self.app_name not in current_allocations:
            raise KeyError(
                f"App {self.app_name} not in current allocations. Currently running apps: {list(current_allocations.keys())}")
