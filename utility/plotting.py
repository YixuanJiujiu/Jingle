...
from Jingle.scheduler.profiling.profiled_info_loader import ProfiledInfoBank
from scheduler.core.base_performance_recorder import BasePerformanceRecorder, BasePerformanceRecorderBank

def get_abs_unit_demand():
    return 0.25
...
ENV_CURVE_KEYS = [RESOURCE_LOSS]
AUTOSCALING_CURVE_KEYS = [LEAF_LOADS, LEAF_UTIL, LEAF_COSTS]
...
ALL_FIELDS_INRUN = [RESOURCE_LOSS, AVG_UTIL]
    ...
    for _, row in df.iterrows():
        ...
        if row_counter >= NUM_INITIAL_ENTRIES_TO_IGNORE:
            curr_data = {EVENT_TIME_PERIOD: row['event_end_time'] - row['event_start_time'],
                         REWARD: row['p99'],
                         # ALLOC_PER_UNIT_LOAD: row['allocs']/row['load'],
                         ALLOC: row['allocs'],
                         # SIGMA: row['sigma'],
...
def process_log_files_in_single_workdir(env, log_files_in_workdir, profiled_info_bank,
    ...
    for log_file_name in log_files_in_workdir:
        ...
        # performance_recorder = BasePerformanceRecorder(
        ...
        performance_recorder = PerformanceRecorder(
            descr=leaf_path, data_logger = None, fields_to_report=['reward', 'event_time_period','alloc', 'load'])
        ...
        experiment_leaf_metrics[leaf_path] = performance_recorder._compute_vals_with_data_batch(
                experiment_data[leaf_path], fields_to_report=['reward', 'event_time_period', 'load'])
...
#     import pdb;pdb.set_trace()
    ...
    return experiment_leaf_metrics
