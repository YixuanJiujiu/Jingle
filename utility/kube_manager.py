...
from Jingle.utility.events import AppUpdateEvent, EventTypes, AppAddEvent
    ...
    def get_deployments(self):
        ...
        deps = {d.metadata.name: d for d in deps.items if
                d.metadata.labels.get(LABEL_IS_WORKLOAD,
                                      "false").lower() == "true"}  # Filter only deployments which are actual workloads. This is to exclude worker and app worker deployments.
        return deps
    ...
    def _create_events(self,
        ...
        for a in added_deployments:
            try:
                ...
                logger.error(f"K8s deployment {a.metadata.name} raised exception when creating AppAddEvent for Jingle. Are all fields correctly added to the k8s labels for this deployment? Error: {exp}. Deployment labels: {a.metadata.labels}.")
                raise exp
        ...
        for r in removed_deployments:
            try:
                ...
                logger.error(
                    f"K8s deployment {a.metadata.name} raised exception when creating AppUpdateEvent(remove) for Jingle. Are all fields correctly added to the k8s labels for this deployment? Error: f{str(exp)}. Deployment labels: f{a.metadata.labels}.")
                raise exp
